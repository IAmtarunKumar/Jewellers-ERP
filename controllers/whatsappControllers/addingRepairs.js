...
const addingRepairs = async (req, res, sessionId, userMessage) => {
  console.log(
    "we are in addingrepairs inner function and logging the rapidapikey",
  ...
  const options = {
    ...
    data: {
      ...
      entities: [
        ...
        {
          var_name: "expected_return_date",
          type: "string",
          description: "expected return date",
        },
        {
          var_name: "repair_cost",
          type: "string",
  ...
  try {
    ...
    issueDesc = response.data.results.issue_desc;
    repairDate = response.data.results.repair_date;
    expectedReturnDate = response.data.results.expected_return_date;
    repairCost = response.data.results.repair_cost;
    ...
    if (response.data.results.repair_date) {
      //the code which is handling input in any format
      // Input date in words
      const dateInWords = response.data.results.repair_date; // or '31 May'
    ...
    if (response.data.results.expected_return_date) {
      //the code which is handling input in any format
      // Input date in words
      const dateInWords = response.data.results.expected_return_date; // or '31 May'
      // Parse the input date
      let parsedDate = parse(dateInWords, "MMMM do", new Date());
      // console.log("parsed date first time", parsedDate);
      if (isNaN(parsedDate.getTime())) {
    ...
    console.log(
      "logging the final repairdate and expected return date",
      repairDate,
      expectedReturnDate
    );
    ...
    if (customerId && productId) {
      //very common case
      if (!issueDesc || !repairDate || !expectedReturnDate || !repairCost)
        return "please fill all the neccessary details as mentioned above to save the repair.";
      const repairId = Math.floor(100000 + Math.random() * 900000);
      ...
      console.log("new repair list saved");
      return "desired repair has been saved successfully!";
      ...
      if (
        ...
        !repairDate ||
        !expectedReturnDate ||
        !repairCost || !productName || !productDesc ||
        !customerName ||
        !customerAddress ||
        !customerContact
      )
